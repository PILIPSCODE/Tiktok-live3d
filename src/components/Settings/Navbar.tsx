...
import { RiFilePaper2Line, RiUserVoiceFill } from "react-icons/ri";
...
function Navbar(props:prop) {
    ...
    return (
        ...
        <div className={`h-screen bg-white overflow-y-scroll text-xl p-8 flex flex-col gap-8 text-black`}>
            <label className='text-4xl'>
                <h1>Settings</h1>
            </label>
            <div className='flex text-white w-full gap-4 max-md:text-sm'>
              <input type="text" className='flex-grow bg-transparent text-black border-b-2  border-black p-2 max-md:w-32 max-md:h-12 focus:outline-none' placeholder='@Tiktok Username'/>
              <button className='bg-black text-white p-3 rounded-md transition-transform duration-300 transform hover:scale-105'>Connect</button>
            </div>
            <ul className='flex flex-col gap-6'>
                {
                    nav_list.map((item:list, index) => (
                        <li className="flex flex-col" key={index}>
                            ...
                            <div className={`flex-grow ${itemNav === item.value? "h-52 opacity-100 p-3 bg-gray-200" :"h-0 opacity-0 "}   rounded-md transition-all duration-300 overflow-y-scroll`}> 
                                <h1>items</h1>
                                <h1>items</h1>
                                <h1>items</h1>
                                <h1>items</h1>
